...
(function () {
  ...
  /* ---------- Formulario de contacto (Formspree opcional) ---------- */
  var form = document.querySelector(".contact-form");
  if (form) {
    form.addEventListener("submit", function (e) {
      var action = form.getAttribute("action") || "";
      if (action.indexOf("formspree.io") === -1) {
        // Sin backend configurado todavía: usamos mailto como respaldo.
        e.preventDefault();
        var name = form.querySelector("#name").value;
        var email = form.querySelector("#email").value;
        var msg = form.querySelector("#message").value;
        var body = encodeURIComponent(msg + "\n\n— " + name + " (" + email + ")");
        window.location.href = "mailto:info@germanosorio.com?subject=Contacto%20desde%20la%20web&body=" + body;
      }
    });
  }
})();
